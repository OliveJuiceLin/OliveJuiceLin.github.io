---
permalink: /
title: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

## 📖 About Me

I am an undergraduate student at Xi'an Jiaotong University, majoring in Computer Science and Technology, supervised by Prof. Minnan Luo.

My research interests are generally open. Now, I focus on **large language model interpretability**, **Knowledge Conflicts in LLMs**, and **Improving LLM reasoning using RL**.

---

## 📝 Publications

[P1] Continuously Steering LLMs Sensitivity to Contextual Knowledge with Proxy Models
- **Yilin Wang**, Heng Wang, Yuyang Bai, Minnan Luo [EMNLP 2025 (Oral)](https://aclanthology.org/2025.emnlp-main.233.pdf)

[P2] Why Knowing Both Hops Is Not Enough: Understanding Two-Hop Generalization in Language Models
- Zili Zhang*, **Yilin Wang***, Heng Wang, Herun Wan, Minnan Luo [Arr under review](https://openreview.net/pdf?id=39v9lWGArD)

[P3] Pseudo-Label Temporal Curriculum Learning for Label-Limited Dynamic Graph
- Shengtao Zhang, Haokai Zhang, Shiqi Lou, Zicheng Wang, Zinan Zeng, **Yilin Wang**, Minnan Luo [arxiv](https://arxiv.org/abs/2504.17641)


---

